...
class TestUnpacker(unittest.TestCase):
    def test_unpack_values_returns_nested_value(self):
        data = {"a": {"b": {"c": 123}}}
        self.assertEqual(unpack_values(data, ("a", "b", "c")), 123)

    def test_unpack_values_returns_none_for_missing_path(self):
        data = {"a": {"b": 1}}
...
class TestGitHubHelpers(unittest.TestCase):
    ...
    def test_make_and_parse_pointer_roundtrip(self):
    ...
    def test_provided_date_invalid_base64_returns_min_datetime(self):
    ...
    def test_auth_mode_legacy_uses_github_token(self):
        with patch.dict(
            os.environ,
    ...
    def test_auth_mode_app_uses_app_installation_token(self):
        with patch.dict(
            os.environ,
    ...
    def test_auth_mode_auto_prefers_app_token(self):
        with patch.dict(
            os.environ,
            {
