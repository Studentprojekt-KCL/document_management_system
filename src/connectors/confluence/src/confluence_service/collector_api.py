...
def _ensure_bind_env() -> None:
    """Map legacy CONFLUENCE_CONNECTOR_PORT to CONCONFLUENCE_BIND_PORT; default bind address."""
    if not (os.environ.get("CONCONFLUENCE_BIND_PORT") or "").strip():
        legacy = (os.environ.get("CONFLUENCE_CONNECTOR_PORT") or "").strip()
        if legacy.isdigit():
            os.environ["CONCONFLUENCE_BIND_PORT"] = legacy
    if not (os.environ.get("CONCONFLUENCE_BIND_ADDR") or "").strip():
        os.environ["CONCONFLUENCE_BIND_ADDR"] = "0.0.0.0"
