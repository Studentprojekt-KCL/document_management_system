...
describe('config.js', () => {
  ...
  describe('Keycloak URL builders', () => {
    ...
    it('keycloakTokenUrl returns correct token endpoint', () => {
      const url = config.keycloakTokenUrl()
      expect(url).toContain('/realms/')
  ...
  describe('storage key names', () => {
    it('exports SESSION_KEY_ACCESS_TOKEN', () => {
      expect(config.SESSION_KEY_ACCESS_TOKEN).toBe('access_token')
    })

    it('exports SESSION_KEY_ID_TOKEN', () => {
      expect(config.SESSION_KEY_ID_TOKEN).toBe('id_token')
    })
    ...
    it('exports SESSION_KEY_REFRESH_TOKEN', () => {
      expect(config.SESSION_KEY_REFRESH_TOKEN).toBe('refresh_token')
    })
