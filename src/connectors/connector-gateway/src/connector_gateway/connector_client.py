...
class ConnectorClient:
    ...
    async def fetch_files_metadata(
        self, pointers: list[str], include_content, include_last_edit_date
    ) -> list[dict]:
        if pointers == []:
            return []
        sorted_pointers: list[list] = self._sort_pointers(pointers)
        response = await self._get_file_from_pointer(
            sorted_pointers, include_content, include_last_edit_date
        )
        return response
    ...
    async def _get_file_from_pointer(
        self, sorted_pointers: list[list], include_content, include_last_edit_date
    ) -> Any | None:
        ...
        for grouped_pointers in sorted_pointers:
            for system in self.source_systems:
                if (
                    self._slice_url_to_host_and_proto(grouped_pointers[0])
                    in system["source_system_url"]
                ):
