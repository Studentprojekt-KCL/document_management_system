...
describe('resolveDateOnly', () => {
  ...
  it('returns Unknown date when no date exists', () => {
    expect(resolveDateOnly({})).toBe('Unknown date')
  })
})
...
describe('resolveSecurityClass', () => {
  ...
  it('returns Pending when missing', () => {
    expect(resolveSecurityClass({})).toBe('Pending')
  })
})
...
describe('useSearchMetadata composable', () => {
  ...
  describe('computed properties with null selectedMatch', () => {
    ...
    it('returns empty strings for all fields', () => {
      ...
      expect(meta.sourceSystem.value).toBe('Unknown')
      expect(meta.previewLink.value).toBe('')
      expect(meta.previewSecurityClass.value).toBe('Pending')
    })
  })
