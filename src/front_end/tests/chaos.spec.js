...
test('chaotic user does not crash the app', async ({ page }) => {
  ...
  if (await loginBtn.isVisible({ timeout: 3_000 }).catch(() => false)) {
    ...
    await expect(page.getByRole('textbox', { name: /Search for documents/i })).toBeVisible({ timeout: 10_000 })
  }
  ...
  const actions = [
    async () => {
      ...
      await page.mouse.click(Math.floor(Math.random() * width), Math.floor(Math.random() * height))
    },
    async () => {
